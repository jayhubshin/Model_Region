...
# 🔧 초기 데이터 파일 설정
# ============================================================
# 아래 경로를 실제 기본 데이터 파일 경로로 변경하세요.
# 앱과 같은 폴더에 두거나, 절대 경로를 지정할 수 있습니다.
DEFAULT_DATA_FILES = [
    "default_data.xlsx",           # 앱과 같은 폴더
    "data/default_data.xlsx",      # data 하위 폴더
    "초기데이터.xlsx",              # 한글 파일명
    "data/초기데이터.xlsx",
]
...
# ============================================================
# 🔥 핵심 변경: 파일에서 초기 데이터 로드하는 함수
# ============================================================

def find_default_data_file():
    """
    기본 데이터 파일을 탐색하여 경로를 반환합니다.
    DEFAULT_DATA_FILES 리스트에 정의된 경로를 순서대로 확인합니다.
    """
...
def load_initial_data_from_file(filepath):
    """
    🔥 엑셀 또는 CSV 파일에서 초기 데이터를 로드하여 대시보드용 DataFrame 생성
    
    지원 형식: .xlsx, .xls, .csv
    
    엑셀 파일의 경우:
        - 4행이 헤더, 5행부터 데이터로 가정
        - A열=사이트ID, H열=주소, AD/AG/AH/AJ열=분류용
        - AM열=경도, AN열=위도, AR열=계약시작일, AS열=계약종료일
        - BA열=모델분류(자동생성), BB열=권역(자동생성)
    
    CSV 파일의 경우:
        - 이미 가공된 데이터(모델분류, 권역 등 포함)를 직접 로드
    """
    ...
    if ext == '.csv':
        ...
        return None, f"지원하지 않는 파일 형식입니다: {ext}"


def _load_from_csv(filepath):
    """CSV 파일에서 대시보드 DataFrame을 로드"""
    try:
        df = pd.read_csv(filepath, encoding='utf-8-sig')

        # 필수 컬럼 확인 및 보정
        required_cols = ['사이트ID', '모델분류', '권역', '주소']
        missing = [c for c in required_cols if c not in df.columns]

        if missing:
            # 컬럼명 매핑 시도 (영문 → 한글)
            col_mapping = {
                'site_id': '사이트ID', 'model': '모델분류',
                'region': '권역', 'address': '주소',
                'lat': '위도', 'lng': '경도', 'longitude': '경도', 'latitude': '위도',
        ...
        for date_col in ['운영계약시작일', '운영계약종료일']:
            if date_col in df.columns:
                df[f'{date_col}_parsed'] = df[date_col].apply(clean_and_parse_date)
                df[f'{date_col}_cleaned'] = df[f'{date_col}_parsed']
        ...
        return None, f"CSV 로드 실패: {str(e)}"


def _load_from_excel(filepath):
    """
    엑셀 파일에서 분류를 수행하고 대시보드 DataFrame을 생성
    (업로드 처리 로직과 동일하지만 진행 바 없이 조용히 실행)
    """
    try:
        ...
        import traceback
        return None, f"엑셀 로드 실패: {str(e)}\n{traceback.format_exc()}"


def create_sample_data():
    """파일이 없을 때 사용하는 최소 샘플 데이터 (폴백)"""
    sample_data = {
        ...
        '주소': [
            '서울특별시 강서구 공항대로 지하 396', '경기도 성남시 분당구 판교역로 166',
            ...
            '서울특별시 강북구 4.19로 100', '충청남도 천안시 동남구 병천면 충절로 1600',
            '인천광역시 계양구 봉오대로744번길 7', '대구광역시 달서구 달구벌대로 1095',
            ...
            '경기도 성남시 중원구 사기막골로 45번길 14', '서울특별시 성북구 정릉로 77',
...
def create_charger_map(filtered_df):
    ...
    region_colors = {
        ...
        '수도권남서': 'purple', '수도권기타': 'cadetblue', '인천??': 'orange',
    ...
    for idx, row in grouped.iterrows():
        ...
        models_text = ', '.join(set([str(m_) for m_ in models]))

        popup_html = f"""
        <div style="width: 320px; font-family: 'Malgun Gothic', Arial, sans-serif;">
            <h4 style="margin: 0 0 12px 0; color: #333; border-bottom: 3px solid {color};
                       padding-bottom: 8px; font-size: 16px;">🏢 {site_id}</h4>
    ...
    for region, clr in region_colors.items():
        if region in grouped['권역'].values:
            ...
            legend_html += f'<p style="margin: 1px 0; font-size: 10px;"><span style="color: {clr}; font-size: 14px;">●</span> {region} ({site_count}개)</p>'
    ...
    if len(valid_dates) > 0:
            ...
            if coord_count > 0:
                ...
                if error:
                    ...
                    with st.expander("🎮 지도 사용 방법"):
                        st.markdown("""
                        **지도 조작:** 마우스 휠로 확대/축소, 드래그로 이동, 클러스터 클릭 시 확대

                        **마커 기능:** 마우스 오버로 간단 정보, 클릭으로 상세 팝업 + 네이버 지도 링크

                        **시각화:** ⚡ 빨간 번개 = 급속 포함, 🔌 파란 플러그 = 완속만, 색상 = 권역별 구분
                        """)
            else:
                st.warning("⚠️ 좌표 데이터가 없습니다.")
        else:
            st.warning("⚠️ 좌표 데이터 컬럼이 없습니다.")
        ...
        with col1:
            ...
            fig_region_pie = px.pie(
                region_counts, values='수량', names='권역',
                title='권역별 충전기 비율', hole=0.4
            )
        ...
        fig_heatmap = px.imshow(
            ...
            x=crosstab_filtered.index, y=crosstab_filtered.columns,
            color_continuous_scale='RdYlGn', aspect="auto",
            title='권역별 주요 모델 분포 (상위 12개 모델)', text_auto=True
        )
        ...
        with col1:
            ...
            st.download_button(
                label="📊 권역×모델 현황표 CSV", data=csv_pivot,
        ...
        with col2:
            ...
            st.download_button(
                label="📋 필터링된 전체 데이터 CSV", data=csv_filtered,
...
"""
            ...
            st.download_button(
                label="📄 요약 리포트 TXT", data=summary_report,
        ...
        unknown_regions = filtered_df[filtered_df['권역'].isin(problematic_regions)]
...
def show_classification_info():
    ...
    with subtab4:
        ...
        st.success("""
        **🗺️ 지도 기능:**
        AM열(경도), AN열(위도) 좌표 기반 위치 표시 / 사이트ID 기준 그룹화 /
        클릭 시 네이버 지도 연동
        """)


# ============================================================
# 🔥 핵심 변경: main() 함수에서 파일 기반 초기 로드
# ============================================================

def main():
    # =====================================================
    # 세션 상태 초기화: 파일 우선 → 샘플 데이터 폴백
    # =====================================================
    if 'processed_df' not in st.session_state:
        ...
        if default_file:
            ...
            if df is not None and error is None:
                ...
                st.session_state.default_file_loaded = default_file
            else:
                # 파일 로드 실패 시 샘플 데이터로 폴백
    ...
    **AR열/AS열**의 날짜 데이터를 정리하여 **AM열(경도)/AN열(위도)** 기반으로 지도에서 충전기 위치를 확인할 수 있습니다.
    ...
    if st.session_state.get('default_file_loaded'):
        st.success(f"📂 **기본 데이터 파일 자동 로드 완료:** `{st.session_state.default_file_loaded}`")
    elif st.session_state.get('file_load_error'):
        st.warning(f"⚠️ 기본 파일 로드 실패: {st.session_state.file_load_error}")
    ...
    with tab1:
        # 🔥 기본 데이터 파일 설정 섹션
        with st.expander("⚙️ 기본 데이터 파일 설정", expanded=False):
            st.markdown("""
            **앱 시작 시 자동으로 로드할 기본 데이터 파일을 설정합니다.**

            아래 경로 중 하나에 파일을 배치하면 앱 실행 시 자동으로 로드됩니다:
            """)

            for i, path in enumerate(DEFAULT_DATA_FILES):
                exists = "✅ 파일 존재" if os.path.exists(path) else "❌ 파일 없음"
                st.code(f"{i + 1}. {path}  →  {exists}")

            st.markdown("---")

            # 🔥 사이드바에서 기본 파일 직접 업로드
            st.markdown("**또는 여기서 기본 데이터 파일을 업로드하세요:**")
            default_upload = st.file_uploader(
                "기본 데이터 파일 (xlsx/csv)",
                type=['xlsx', 'xls', 'csv'],
                key='default_file_uploader',
                help="이 파일은 앱 시작 시 자동으로 로드되는 기본 데이터로 사용됩니다."
            )

            if default_upload is not None:
                if st.button("📂 기본 데이터로 설정", type="primary"):
                    # 파일 저장
                    save_path = DEFAULT_DATA_FILES[0]  # 첫 번째 경로에 저장
                    with open(save_path, 'wb') as f:
                        f.write(default_upload.read())

                    # 즉시 로드
                    df, error = load_initial_data_from_file(save_path)
                    if df is not None:
                        st.session_state.processed_df = df
                        st.session_state.is_sample_data = False
                        st.session_state.default_file_loaded = save_path
                        st.success(f"✅ 기본 데이터 파일이 설정되었습니다: `{save_path}` ({len(df):,}행)")
                        st.rerun()
                    else:
        ...
        if st.session_state.get('is_sample_data', False):
            st.info("💡 **현재 샘플 데이터가 로드되어 있습니다.** 위 '기본 데이터 파일 설정'에서 파일을 등록하거나, 아래에서 파일을 업로드하세요.")
        ...
        if uploaded_file is not None:
            ...
            if st.button("🚀 모델분류 시작", type="primary", use_container_width=True):
                ...
                result = process_excel_file_with_progress(
                    file_bytes, title_container, progress_bar, status_text
                )
